...
import com.sandrew.boot.core.common.ObjectUtils;
import com.sandrew.boot.mapper.session.SessionMapper;
...
public class MySqlSessionValidationScheduler implements SessionValidationScheduler, Runnable
{
	...
	public void run()
	{
		...
		int start = 0; //起始记录
		int size = 20; //每页大小
		SessionPager pager = new SessionPager(start, size);
		List<TtSessionPO> sessionList = sessionMapper.pageQuerySession(pager);
		while (sessionList.size() > 0)
		{
			for (TtSessionPO sessionPO : sessionList)
			{
				try
				{
					String sessionStr = sessionPO.getSession();
					Session session = (Session) ObjectUtils.unserialize(Base64.decode(sessionStr));
			...
			start = start + size;
			pager = new SessionPager(start, size);
			sessionList = sessionMapper.pageQuerySession(pager);
		}
