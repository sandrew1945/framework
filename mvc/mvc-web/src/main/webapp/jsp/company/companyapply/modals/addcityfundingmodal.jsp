<%@ page language="java" contentType="text/html; charset=utf-8"
	pageEncoding="utf-8"%>
		<div class="modal-header">
			...
			<h4 class="modal-title">添加市拨款明细</h4>
		</div>
		<div class="portlet-body">
			<form action="#" id="fmcityfunding" name="fmstatefunding" class="form-horizontal" method="post">
			<!-- BEGIN ROLE LIST PORTLET-->
			<div class="portlet light">
				<div class="portlet-title">
					<div class="caption font-grey-gallery">
						<i class="icon-list font-grey-gallery"></i>
						<span class="caption-subject bold"> 填写市拨款信息</span>
					</div>
					<div class="actions">
...
var cityFundingValOpt = {
   		formId : "fmcityfunding",
   		formRoles : {
   						source: {
			                required: true
			            },
   		...
   		formCallBack: {
   					url:  "companyapply/createCityFundingDetail",	// 验证通过调用的后台连接
   					type: "POST",  
   					dataType: "json", 
   					success: function(ajaxResult)
   					{
   						// altConfirm(alt_param);							// 处理成功的回调
   						AjaxAnyWhere.handleResult(ajaxResult, handleCityFundingSuccess);
...
function cityFundingSubmit(){
	$("#fmcityfunding").submit();
}
// 处理成功回调方法
var handleCityFundingSuccess = function ()
{
	// 关闭弹出层
	$cityfundingModal.modal('hide');
	// 刷新国家拨款列表
	DataGrid.__extQueryAll__(cityFundingOpt);
}

jQuery(document).ready(function() {
	CommonFormVaildate.init(cityFundingValOpt);
});
</script>
